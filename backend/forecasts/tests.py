...
class SeasonalNaivePredictTests(unittest.TestCase):
    ...
    def test_exact_match_predicts_correctly(self):
        ...
        df = _make_df([
            {"day_of_week": 2, "time_window": "18:00-19:00", "observed_reservations": 8,
             "unreserved_stock": 2, "observed_no_show": 0},
        ])
    ...
    def test_falls_back_to_category_when_no_exact_slot(self):
        ...
        df = _make_df([
            {"day_of_week": 5, "time_window": "10:00-11:00", "observed_reservations": 6,
             "unreserved_stock": 4, "observed_no_show": 0},
        ])
        preds, _ = seasonal_naive_predict(df, self._make_inp())
        self.assertGreater(preds, 0)

    def test_empty_df_returns_zeros(self):
        df = pd.DataFrame(columns=["category", "day_of_week", "time_window",
                                    "observed_reservations", "observed_no_show",
                                    "unreserved_stock", "record_id"])
        preds, no_show = seasonal_naive_predict(df, self._make_inp())
...
class MovingAveragePredictTests(unittest.TestCase):
    ...
    def test_uses_last_window_records(self):
        ...
        old_rows = [{"observed_reservations": 5, "unreserved_stock": 5, "observed_no_show": 0}] * 5
        new_rows = [{"observed_reservations": 10, "unreserved_stock": 0, "observed_no_show": 0}] * 10
...
class SimilarityPredictTests(unittest.TestCase):
    ...
    def test_basic_prediction(self):
        df = _make_df([
            {"day_of_week": 2, "time_window": "18:00-19:00", "observed_reservations": 8,
             "unreserved_stock": 2, "observed_no_show": 0, "weather_flag": 0},
        ])
    ...
    def test_price_sensitivity_lowers_prediction_for_high_price(self):
        ...
        df = _make_df([
            {"price": 5.0, "day_of_week": 2, "time_window": "18:00-19:00",
    ...
    def test_weather_weighting(self):
        ...
        df = _make_df([
            {"weather_flag": 1, "observed_reservations": 2, "unreserved_stock": 8,
             "observed_no_show": 0},   # bad weather → low uptake
            {"weather_flag": 0, "observed_reservations": 9, "unreserved_stock": 1,
             "observed_no_show": 0},   # good weather → high uptake
        ])
...
class EvaluateBaselinesTests(unittest.TestCase):
    def test_returns_all_three_approaches(self):
        df = _make_df([
            {"observed_reservations": 8, "unreserved_stock": 2, "observed_no_show": 0},
            {"observed_reservations": 7, "unreserved_stock": 3, "observed_no_show": 1},
            {"observed_reservations": 6, "unreserved_stock": 4, "observed_no_show": 0},
        ])
    ...
    def test_rmse_is_non_negative(self):
        df = _make_df([
            {"observed_reservations": 8, "unreserved_stock": 2, "observed_no_show": 1},
        ] * 5)
        result = evaluate_baselines(df)
        for m in result.values():
    ...
    def test_single_row_returns_n_zero(self):
        ...
        df = _make_df([{"observed_reservations": 8, "unreserved_stock": 2,
                         "observed_no_show": 0}])
        result = evaluate_baselines(df)
        for m in result.values():
...
class GenerateRecommendationTests(unittest.TestCase):
    ...
    def test_returns_action_and_rationale(self):
        rec = generate_recommendation(self._make_inp(), predicted_reservations=8.0,
                                       no_show_prob=0.1, n_records=10)
        self.assertIn("action", rec)
        self.assertIn("rationale", rec)

    def test_recommends_reduction_when_low_fill_rate(self):
        # 5 expected from 12 posted = 42% fill rate → should suggest fewer
        rec = generate_recommendation(self._make_inp(no_bundles=12),
                                       predicted_reservations=5.0,
                                       no_show_prob=0.05, n_records=20)
    ...
    def test_elevated_no_show_mentioned_in_rationale(self):
        rec = generate_recommendation(self._make_inp(), predicted_reservations=9.0,
                                       no_show_prob=0.35, n_records=15)
        self.assertIn("no-show", rec["rationale"].lower())

    def test_weather_flag_mentioned_in_rationale(self):
        rec = generate_recommendation(self._make_inp(weather_flag=1),
                                       predicted_reservations=9.0,
                                       no_show_prob=0.1, n_records=15)
        self.assertIn("weather", rec["rationale"].lower())

    def test_price_mentioned_when_provided(self):
        rec = generate_recommendation(self._make_inp(price=4.50),
                                       predicted_reservations=9.0,
                                       no_show_prob=0.1, n_records=15)
